...
function openEmployeeModal(id) {
  ...
  document.getElementById("emp-tax").value = 16;
  document.getElementById("emp-pension").value = 8;
...
// ---------------------------------------------------------------------
// Live breakdown preview — recalculates as the admin types, so the
// Basic/Housing/Transport/Utility split and estimated net pay update
// immediately when a Gross Pay figure (or any rate) is entered.
// ---------------------------------------------------------------------
const BREAKDOWN_FIELD_IDS = ["emp-base", "emp-bonus", "emp-driver", "emp-airtime", "emp-other-allowance", "emp-tax", "emp-pension", "emp-hmo", "emp-deductions"];
...
function updateBreakdownPreview() {
  ...
  const fakeEmp = {
    ...
    tax_rate: document.getElementById("emp-tax").value,
  ...
  el.innerHTML = `
    ...
    <div class="breakdown-row"><span>Transport allowance (20%)</span><span>${formatMoney(calc.transport)}</span></div>
    <div class="breakdown-row"><span>Utility allowance (10%)</span><span>${formatMoney(calc.utility)}</span></div>
    <div class="breakdown-row total"><span>Gross pay</span><span>${formatMoney(calc.gross)}</span></div>
    ...
    <div class="breakdown-row"><span>Bonus + allowances</span><span>+${formatMoney(calc.allowancesTotal)}</span></div>
    <div class="breakdown-row total net"><span>Estimated net pay</span><span>${formatMoney(calc.net)}</span></div>
    <div class="breakdown-row muted"><span>Employer pension (10%, ref. only)</span><span>${formatMoney(calc.employerPension)}</span></div>
    <div class="breakdown-row muted"><span>Leave allowance (annual, ref. only)</span><span>${formatMoney(calc.leaveAllowanceAnnual)}</span></div>
  `;
}
