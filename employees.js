...
function renderEmployeesTab() {
  ...
  tbody.innerHTML = rows.map((e) => {
    ...
    if (currentEmpTab === "active") {
      actions = `
        <button class="btn-link admin-only" data-edit="${e.id}">Edit</button>
        <button class="btn-link danger admin-only" data-exit="${e.id}">Mark exited</button>
        <button class="btn-link danger admin-only" data-delete="${e.id}">Delete</button>`;
    } else if (currentEmpTab === "archived") {
      actions = `
        <button class="btn-link admin-only" data-edit="${e.id}">Edit</button>
        <button class="btn-link admin-only" data-reinstate="${e.id}">Reinstate</button>
        <button class="btn-link danger admin-only" data-delete="${e.id}">Delete</button>`;
    } else {
      actions = `<button class="btn-link admin-only" data-restore="${e.id}">↺ Restore</button>`;
    }
    return `
    <tr>
...
function openEmployeeModal(id) {
  ...
  document.getElementById("emp-pension").value = 8.52347427456386;
  ...
  if (isEdit) {
    ...
    if (emp) {
      ...
      document.getElementById("emp-driver").value = emp.driver_allowance || 0;
      document.getElementById("emp-airtime").value = emp.airtime_allowance || 0;
      document.getElementById("emp-other-allowance").value = emp.other_allowance || 0;
      document.getElementById("emp-pension").value = emp.pension_rate;
      document.getElementById("emp-voluntary").value = emp.voluntary_contribution || 0;
      document.getElementById("emp-other-reliefs").value = emp.other_tax_reliefs || 0;
      document.getElementById("emp-surcharge").value = emp.surcharge || 0;
      document.getElementById("emp-loan").value = emp.loan_repayment || 0;
...
// ---------------------------------------------------------------------
// Live breakdown preview — recalculates as the admin types, matching
// the official PAYE Calculator template exactly (CRA + progressive
// tax bands), so the full breakdown and estimated net pay update
// immediately as figures are entered.
// ---------------------------------------------------------------------
const BREAKDOWN_FIELD_IDS = [
  "emp-base", "emp-bonus", "emp-driver", "emp-airtime", "emp-other-allowance",
  "emp-pension", "emp-voluntary", "emp-other-reliefs", "emp-surcharge", "emp-loan",
  "emp-hmo", "emp-deductions",
];

function updateBreakdownPreview() {
  const el = document.getElementById("breakdown-preview");
  if (!el) return;

  const fakeEmp = {
    base_salary: document.getElementById("emp-base").value,
    bonus: document.getElementById("emp-bonus").value,
    driver_allowance: document.getElementById("emp-driver").value,
    airtime_allowance: document.getElementById("emp-airtime").value,
    other_allowance: document.getElementById("emp-other-allowance").value,
    pension_rate: document.getElementById("emp-pension").value,
    voluntary_contribution: document.getElementById("emp-voluntary").value,
    other_tax_reliefs: document.getElementById("emp-other-reliefs").value,
    surcharge: document.getElementById("emp-surcharge").value,
    loan_repayment: document.getElementById("emp-loan").value,
    hmo_deduction: document.getElementById("emp-hmo").value,
    other_deductions: document.getElementById("emp-deductions").value,
  };

  if (typeof calcPayslip !== "function") return;
  const calc = calcPayslip(fakeEmp);

  el.innerHTML = `
    <div class="breakdown-row"><span>Basic salary (54%)</span><span>${formatMoney(calc.basic)}</span></div>
    <div class="breakdown-row"><span>Housing allowance (16%)</span><span>${formatMoney(calc.housing)}</span></div>
    <div class="breakdown-row"><span>Transport allowance (10%)</span><span>${formatMoney(calc.transport)}</span></div>
    <div class="breakdown-row"><span>Utility allowance (10%)</span><span>${formatMoney(calc.utility)}</span></div>
    <div class="breakdown-row"><span>Leave allowance (10%)</span><span>${formatMoney(calc.leave)}</span></div>
    <div class="breakdown-row"><span>Others</span><span>${formatMoney(calc.others)}</span></div>
    <div class="breakdown-row total"><span>Gross pay (monthly)</span><span>${formatMoney(calc.gross)}</span></div>
    <div class="breakdown-row"><span>Employee pension</span><span>−${formatMoney(calc.employeePension)}</span></div>
    <div class="breakdown-row"><span>Tax (PAYE)</span><span>−${formatMoney(calc.tax)}</span></div>
    <div class="breakdown-row"><span>Voluntary + surcharge + loan</span><span>−${formatMoney(calc.voluntaryContribution + calc.surcharge + calc.loanRepayment)}</span></div>
    <div class="breakdown-row"><span>HMO + other deductions</span><span>−${formatMoney(calc.hmo + calc.otherDeductions)}</span></div>
    <div class="breakdown-row"><span>Bonus + driver + airtime</span><span>+${formatMoney(calc.allowancesTotal)}</span></div>
    <div class="breakdown-row total net"><span>Estimated net pay</span><span>${formatMoney(calc.net)}</span></div>
    <div class="breakdown-row muted"><span>Annual gross pay (ref.)</span><span>${formatMoney(calc.annualGrossPay)}</span></div>
    <div class="breakdown-row muted"><span>Employer pension (ref. only)</span><span>${formatMoney(calc.employerPension)}</span></div>
    <div class="breakdown-row muted"><span>Charge income / annual (ref.)</span><span>${formatMoney(calc.chargeIncome)}</span></div>
  `;
}

BREAKDOWN_FIELD_IDS.forEach((id) => {
  const field = document.getElementById(id);
  if (field) field.addEventListener("input", updateBreakdownPreview);
});
...
employeeForm.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    driver_allowance: Number(document.getElementById("emp-driver").value || 0),
    airtime_allowance: Number(document.getElementById("emp-airtime").value || 0),
    other_allowance: Number(document.getElementById("emp-other-allowance").value || 0),
    pension_rate: Number(document.getElementById("emp-pension").value || 0),
    voluntary_contribution: Number(document.getElementById("emp-voluntary").value || 0),
    other_tax_reliefs: Number(document.getElementById("emp-other-reliefs").value || 0),
    surcharge: Number(document.getElementById("emp-surcharge").value || 0),
    loan_repayment: Number(document.getElementById("emp-loan").value || 0),
