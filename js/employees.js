...
function renderEmployeesTab() {
  ...
  const active = allEmployees.filter((e) => !isArchived(e));
  const archived = allEmployees.filter(isArchived);
  ...
  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="6" class="empty-row">${
      currentEmpTab === "active" ? "No active employees. Add your first one above." : "No one archived yet."
    }</td></tr>`;
    return;
  }
