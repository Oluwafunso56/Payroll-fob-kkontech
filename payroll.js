...
function renderPayrollOverview() {
  ...
  const staff = allEmployees.filter((e) => !isArchived(e));
...
document.getElementById("preview-btn").addEventListener("click", () => {
  ...
  const eligible = allEmployees.filter((e) => isEligibleForPeriod(e, period));
  if (!eligible.length) {
    tbody.innerHTML = `<tr><td colspan="7" class="empty-row">No employees are eligible for pay in ${escapeHtml(period)} (check hire/exit dates).</td></tr>`;
    tfoot.hidden = true;
    runBtn.disabled = true;
    return;
  }
...
document.getElementById("run-btn").addEventListener("click", async () => {
  ...
  if (runError) {
    ...
    statusEl.textContent = runError.message.includes("duplicate")
      ? `A payroll run for ${period} already exists — see History.`
