...
function renderPayrollOverview() {
  ...
  if (!staff.length) {
    tbody.innerHTML = `<tr><td colspan="11" class="empty-row">No active staff yet — add employees first.</td></tr>`;
    tfoot.hidden = true;
    return;
  }
  ...
  tbody.innerHTML = rows.map(({ emp, calc }) => `
    <tr>
      ...
      <td><button class="btn-link danger" data-delete="${emp.id}">Delete</button></td>
    </tr>
  `).join("");
  ...
  tbody.innerHTML = data.map((run) => `
    <tr class="run-row" data-run="${run.id}" data-period="${escapeHtml(run.period)}">
      ...
      <td class="row-actions">
        ...
        <button class="btn-link danger" data-delete-run="${run.id}" data-period-label="${escapeHtml(run.period)}">Delete</button>
      </td>
    </tr>
  `).join("");
  ...
  tbody.innerHTML = data.map((p) => `
    <tr>
      ...
      <td class="row-actions">
        ...
        <button class="btn-link danger" data-delete-payslip="${p.id}" data-payslip-name="${escapeHtml(p.full_name)}">Delete</button>
      </td>
    </tr>
  `).join("");
